...
public class BlockGrapes extends BlockFlower
{

    protected BlockGrapes(int i, int j)
    {
        super(i, j);
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int l, float f)
    {
        ...
        for(int i1 = 0; i1 < 2; i1++)
        {
            if(world.rand.nextInt(15) <= l)
            {
                ...
                EntityItem entityitem = new EntityItem(world, (float)i + f2, (float)j + f3, (float)k + f4, new ItemStack(Block.grapePlant));
    ...
    public int idDropped(int i, Random random)
    {
        if(i == 7)
        {
            return Item.grapes.shiftedIndex;
        } else
        {
            return -1;
        }
